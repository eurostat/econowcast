...
# Set the Google
kwd1 <- "uncertainty"
kwd2 <- "economic uncertainty"
kwd3 <- "financial uncertainty"
kwd4 <- "policy uncertainty"
reg <- ""               # Region
...
kwd <- kwd3
stp <- "web" 
mtitle5 <- paste(kwd, stp, sep=", ")
gt <- gtrends(kwd, reg, tsd, stp, cct)
x5 <- gt$interest_over_time$hits
d5 <- gt$interest_over_time$date

kwd <- kwd3
stp <- "news" 
mtitle6 <- paste(kwd, stp, sep=", ")
gt <- gtrends(kwd, reg, tsd, stp, cct)
x6 <- gt$interest_over_time$hits
d6 <- gt$interest_over_time$date

kwd <- kwd4
stp <- "web" 
mtitle7 <- paste(kwd, stp, sep=", ")
gt <- gtrends(kwd, reg, tsd, stp, cct)
x7 <- gt$interest_over_time$hits
d7 <- gt$interest_over_time$date

kwd <- kwd4
stp <- "news" 
mtitle8 <- paste(kwd, stp, sep=", ")
gt <- gtrends(kwd, reg, tsd, stp, cct)
x8 <- gt$interest_over_time$hits
d8 <- gt$interest_over_time$date
...
plot(d, z5, type="l", main=mtitle5, xlab="", ylab="")
plot(d, z6, type="l", main=mtitle6, xlab="", ylab="")
plot(d, z7, type="l", main=mtitle7, xlab="", ylab="")
plot(d, z8, type="l", main=mtitle8, xlab="", ylab="")
par(mfrow=c(1,1))
dev.off()
...
# Compare the cleaned series to EPU
rcc <- xmr[,7]
per1 <- d<as.Date("2012-01-01")
per2 <- d>=as.Date("2012-01-01")
c1 <- round(cor(scc[per1], rcc[per1]),2)
...
pdf(file=paste("out/", su, "eEPU.pdf", sep=""), width=11.7, height=8.3)
par(mar=c(5,4,4,5)+.1)
plot(d, scc, type="l", xlab=sub, ylab="Google",
     main=paste(mainkwd, ", clean vs EPU", sep=""), col="blue")
par(new=TRUE)
plot(d, rcc, type="l", xlab="", ylab="", xaxt="n",yaxt="n",
     col="red")
axis(side = 4)
mtext(side = 4, line = 3, "EPU")
dev.off()

# Compare the cleaned series to vix
rcc <- xmr[,8]
per1 <- d<as.Date("2012-01-01")
per2 <- d>=as.Date("2012-01-01")
c1 <- round(cor(scc[per1], rcc[per1]),2)
...
pdf(file=paste("out/", su, "eVIX.pdf", sep=""), width=11.7, height=8.3)
par(mar=c(5,4,4,5)+.1)
plot(d, scc, type="l", xlab=sub, ylab="Google",
     main=paste(mainkwd, ", clean vs VIX", sep=""), col="blue")
par(new=TRUE)
plot(d, rcc, type="l", xlab="", ylab="", xaxt="n",yaxt="n",
     col="red")
axis(side = 4)
mtext(side = 4, line = 3, "VIX")
dev.off()
