...
target <- "target.ei_isin_m.UK"

# Load the data
xw <- read.csv(paste("data/", cocode, "/FINAL-WEEKLY.csv", sep=""))
xm <- read.csv(paste("data/", cocode, "/FINAL-MONTHLY.csv", sep=""))
xT <- read.csv(paste("data/", cocode, "/FINAL-TARGETS.csv", sep=""))
...
# Extract Financials
xF <- xw[,c(4:6, 13)]; cF <- cw[,c(4:6, 13)]

# Extract Bonds
xB <- xw[, 7:12]; cB <- cw[, 7:12]

# Create final XB
isel <- 1:NCOL(xB)
xBF <- cbind(xB[,isel], xB[,5]-xB[,1], xB[,5]-xB[,2],
             xB[,6]-xB[,1], xB[,6]-xB[,3])
cBF <- cbind(cB[,isel], matrix(c(0,0,1),3,4))
colnames(xBF) <- c(colnames(xB)[isel], "10y-3m", "10y-1y", "30-3m", "30-3y")
...
write.csv(colnames(xwF), paste("FINAL-WEEKLY-SEL-", target, ".csv",sep=""))
write.csv(colnames(xmF), paste("FINAL-MONTHLY-SEL-", target, ".csv",sep=""))
...
beep(10)
save.image(paste("MAINOUT-",target, ".Rdata", sep=""))
